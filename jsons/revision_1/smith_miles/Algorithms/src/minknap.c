...
#define MAXSTATES 600000 //The size here will limit the speed of the algorithm, but the algorithm will fail for some problems if maxstates is too large, this must be considered when running
...
long minknap_wrap(int n, int *p, int *w, int *x, int c, double *timerecord)
{
	...
	while (tmptime < MIN_TEST_TIME)
	{
		...
		{
			z = minknap(n, p, w, x, c);
			pallocrem = pallocrem + palloctime;
		}
		...
		{
			pallocrem = 0;
			no_runs = no_runs * 10; // if the measured time wasn't long enough
		}
		std::cout << tmptime << ";" << pallocrem << "\n";
	}

	*timerecord = (double)tmptime / no_runs;
