...
#define MAXSTATES  600000    /*Deals with how many states gthe algorithm can assess at once. Bigger can handle bigger problems theoretically, but slows down code. Keep same on Combo and Minknap for fair assessment of instances*/
...
static void initfirst(allinfo *a, stype pb, stype wb)
{
	...
	struct timespec pallocstart, pallocfinish;
	
	clock_gettime(CLOCK_MONOTONIC, &pallocstart);

	d->set1 = (state *) palloc(sizeof(state), MAXSTATES);
	
	clock_gettime(CLOCK_MONOTONIC, &pallocfinish);
	
	palloctime = (pallocfinish.tv_sec - pallocstart.tv_sec);
	palloctime += (pallocfinish.tv_nsec - pallocstart.tv_nsec) / 1000000000.0;
	//palloc is a problem. It is a simple function but an unknown reason suffers a serious time increase with growing MAXSTATES. To resolve this, since this time is unrelated to algorithm performance, the time taken to run this code section is taken and take from the total time for each run
...
long combo_wrap(int n, int *p, int *w, int *x, int c, double *timerecord)
{
	...
	while (tmptime < MIN_TEST_TIME)
	{
		clock_gettime(CLOCK_MONOTONIC, &start);
		for (int t = 0; t < noRuns; t++)

		{
			zzz = combo(f, l, c, lbi, ubi, def, relx); //zzz is the solution from the algorithm
			pallocrem = pallocrem + palloctime; //deals with palloc time for multiple runs
		}
		...
		if (tmptime < MIN_TEST_TIME)
		{
			pallocrem = 0;
			noRuns = noRuns * 10; // if the measured time wasn't long enough, run multiple time to keep errors low
		}
		std::cout << tmptime << ";" << pallocrem << "\n";
	}

	*timerecord = (double)tmptime / noRuns; //global variable, outputs algorithm runtime
